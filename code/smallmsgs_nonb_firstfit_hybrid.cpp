...
int main(int argc, char* argv[]) {
    ...
    while (finished == false) {
        ...
        for (uint32_t ii = 0; ii < max_conflicts; ii+= superstep) { //iterate for as long as the process with the most conflicts
          ...
          //coloring is done here, communication starts
            ...
            if (counter == 1) { //if first round, take advantage of contiguous layout
              ...
              std::cout << "rank " << rank << " reassigned " << reassigned[0] << std::endl;
        ...
        conflicts.clear();
        for (auto &i : oldconflicts) { // check everything
            uint32_t start = localrowpointers[i];
            uint32_t end = (localchunksize - 1 == i) ? localnedges : localrowpointers[i + 1];
    ...
    if (rank == 0) {
        ...
        std::ofstream results("results_smallmsg_nonb_firstfit_hybrid.csv", std::ios_base::app | std::ios_base::out);
        if (stats == 0) {
