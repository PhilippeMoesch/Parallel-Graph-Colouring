...
int main(int argc, char* argv[]) {
    ...
    std::cout << "rank " << rank << " num threads " << omp_get_max_threads() << std::endl;
    ...
    while (finished == false) {
        ...
        if (conf_size < 50) { //if we have less than 50 vertices to color, do it sequentially per rank (to avoid race conditions)
          n_threads = 1;
        }
        ...
        for (uint32_t ii = 0; ii < max_conflicts; ii+= superstep) { //iterate for as long as the process with the most conflicts
          if (ii < conf_size) {
            ...
            for (uint32_t jj = ii; jj < (ii + superstep); jj++) {
              if (jj < conf_size) { //if we still have conflicts to resolve
                ...
                if (counter > 1) {
                  reassigned[jj-ii+1] = offset + i; //index according to parallel loop index.
                }
                n_colored += 1;
        ...
        for (auto &i : oldconflicts) { // All boundary vertices
            ...
            for (uint32_t j = start; j < end; j++) {
                ...
                if (vertexcolor == owncolor && (i + offset) <= neighbor) { // The smaller rank recolors
                    conflicts.push_back(i); //i+offset?
                    break;
                }
            }
        }
    ...
    if (rank == 0) {
        ...
        */
        ...
        std::ofstream results("results_smallmsg_blocking_firstfit_hybrid.csv", std::ios_base::app | std::ios_base::out);
        if (stats == 0) {
