...
int smallmsgs_nonb_staggered(uint32_t* rowpointers, uint32_t* edges, uint32_t n, int rank, int size, int nedges, int superstep, int estimate, int stats, std::string filename) {
    ...
    if (rank == 0) {
        ...
        std::ofstream results("results_smallmsg_nonb_staggered.csv", std::ios_base::app | std::ios_base::out);
        if (stats == 0) {
...
int smallmsgs_nonb_firstfit(uint32_t* rowpointers, uint32_t* edges, uint32_t n, int rank, int size, int nedges, int superstep, int estimate, int stats, std::string filename) {
...
int smallmsgs_nonb_avoiding(uint32_t* rowpointers, uint32_t* edges, uint32_t n, int rank, int size, int nedges, int superstep, int estimate, int stats, std::string filename) {
    ...
    while (finished == false) {
        ...
        for (uint32_t ii = 0; ii < max_conflicts; ii++) { //iterate for as long as the process with the most conflicts
            if (ii < conf_size) { //if we still have conflicts to resolve
                ...
                int init = std::ceil(rank * estimate / size);
                int ending = std::ceil((rank + 1) * estimate / size);
                int prev = std::ceil((rank - 1) * estimate / size);
                int mid = (init + prev) / 2;
                int init2 = std::floor(rank * estimate / size);
                bool found = false;

                //modified STAGGERED first fit assignment

                // assign smallest permissible color from init to end
                for (int k = init; k <= ending; k++) {
                    if (forbidden[k] != i) {
                        colors[offset + i] = k;
                        if (counter > 1) {
                            reassigned_colors[n_colored - 1] = k;
                ...
                if (!found && (rank > 0)) {
                    // assign smallest permissible color from init-1 to 0, backwards
                ...
                if (!found) {
                    // assign smallest permissible color from estimate to ending, backwards
                    for (int k = estimate; k > ending; k--) {
                        if (forbidden[k] != i) {
                            colors[offset + i] = k;
                            if (counter > 1) {
                                reassigned_colors[n_colored - 1] = k;
    ...
    if (rank == 0) {
        ...
        std::ofstream results("results_smallmsg_nonb_avoiding.csv", std::ios_base::app | std::ios_base::out);
        if (stats == 0) {
            results << filename << "," << size << "," << n << "," << nedges << "," << (endtime - starttime) <<
                    "," << counter << "," << max << "," << superstep << "," << valid << "\n";
        } else {
...
int smallmsgs_blocking_staggered(uint32_t* rowpointers, uint32_t* edges, uint32_t n, int rank, int size, int nedges, int superstep, int estimate, int stats, std::string filename) {
    ...
    while (finished == false) {
        ...
        for (uint32_t ii = 0; ii < max_conflicts; ii++) { //iterate for as long as the process with the most conflicts
            ...
            //
            if ((((ii + 1) % superstep) == 0) || (ii == max_conflicts - 1)) { //always if mod superstep == 0 or last iteration
    ...
    if (rank == 0) {
        ...
        std::ofstream results("results_smallmsg_blocking_staggered.csv", std::ios_base::app | std::ios_base::out);
        if (stats == 0) {
            results << filename << "," << size << "," << n << "," << nedges << "," << (endtime - starttime) <<
                    "," << counter << "," << max << "," << superstep << "," << valid << "\n";
        } else {
...
int smallmsgs_blocking_firstfit(uint32_t* rowpointers, uint32_t* edges, uint32_t n, int rank, int size, int nedges, int superstep, int estimate, int stats, std::string filename) {
    ...
    if (rank == 0) {
        ...
        std::ofstream results("results_smallmsg_blocking_firstfit.csv", std::ios_base::app | std::ios_base::out);
        if (stats == 0) {
            results << filename << "," << size << "," << n << "," << nedges << "," << (endtime - starttime) <<
                    "," << counter << "," << max << "," << superstep << "," << valid << "\n";
        } else {
 ...
 *
 * Small message version: only the changed colors are exchanged at every communication step.
 * avoiding fit: each process starts from a different first color. After filling his share,
 * we proceed backwards to 0 from the start index, and wrap around to estimate when hitting 0.
 *
 * The basic data structures are 2 arrays: one contiguous array of edge connections, and a rowpointer that
 * specifies at which position in the edge array the next vertex starts. These 2 data structures
 * are distributed to the MPI ranks and each rank works on their portion of these arrays.
 ...
 *
 */
int smallmsgs_blocking_avoiding(uint32_t* rowpointers, uint32_t* edges, uint32_t n, int rank, int size, int nedges, int superstep, int estimate, int stats, std::string filename) {
    // Making sure all ranks wait until the graph is loaded
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        std::ofstream results("results_smallmsg_blocking_avoiding.csv", std::ios_base::app | std::ios_base::out);
        if (stats == 0) {
            results << filename << "," << size << "," << n << "," << nedges << "," << (endtime - starttime) <<
                    "," << counter << "," << max << "," << superstep << "," << valid << "\n";
        } else {
...
int main(int argc, char* argv[]) {
    ...
    int superstepArray[] = {100, 500, 1000, 5000};
    int i = 0;
    for (int j = 0; j < 5; j++) {
        do {
            smallmsgs_blocking_avoiding(rowpointers, edges, n, rank, size, nedges, superstepArray[i], estimate, stats, filename);
            smallmsgs_blocking_firstfit(rowpointers, edges, n, rank, size, nedges, superstepArray[i], estimate, stats, filename);
            smallmsgs_blocking_staggered(rowpointers, edges, n, rank, size, nedges, superstepArray[i], estimate, stats, filename);
            smallmsgs_nonb_avoiding(rowpointers, edges, n, rank, size, nedges, superstepArray[i], estimate, stats, filename);
            smallmsgs_nonb_firstfit(rowpointers, edges, n, rank, size, nedges, superstepArray[i], estimate, stats, filename);
            smallmsgs_nonb_staggered(rowpointers, edges, n, rank, size, nedges, superstepArray[i], estimate, stats, filename);
            i++;
        } while (i < 4);
    }
